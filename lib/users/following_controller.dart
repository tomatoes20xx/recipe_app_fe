...
class FollowingController extends PaginatedListController<UserSearchResult> {
  FollowingController({required this.userApi, required this.username});
  ...
  Future<PaginatedResponse<UserSearchResult>> fetchPage(String? cursor) async {
    final res = await userApi.getFollowing(
      username: username,
      limit: limit,
      cursor: cursor,
    );
